...
extern "C"
{
    ...
    extern "C" int _Z18GetField0x3b4ValueP12BattleStruct(BattleStruct*);
    // set day/night time
    ...
    extern "C" bool _Z18TestBitInByteArrayiPhi(void*, void*, int);

    char* func_0205ec34(); 

    extern "C" void _Z14GetVec3ByIndexP8Vec3BA28iPiS1_S1_(LightingInfo*, int, float*, float*, float*);

    extern "C" void _Z11SetFogStateijjt(int, int, int, int);
    // set edge colors
    void _Z19CallWithAddr4000330i(unsigned short*);
    // set fog table
    extern "C" void _Z19CallWithAddr4000360i(void*);

    // do bitmask of something
    extern "C" int _Z24GetCombatWorkFlags0x55f4Pvi(void*, int);

    void* func_ov017_021b8468(void*);
    extern "C" void _Z34SetupCombatantAllocations_021901acv(Struct_ov017_44C8*);
    ...
    Light3DConfig *outLight0, Light3DConfig *outLight1)
{
    ...
    else
    {
        ...
        if (ov17thing != NULL)
        {
            ...
            if (ov0thing != NULL && _Z24GetCombatWorkFlags0x55f4Pvi(ov0thing, 1 << 9))
            {
...
void LightingManager::ModelTransformTintBrightnessContrast(NSBXXInternalModel *model)
{
    ...
    int index;
    if (zone->currentZoneID_ == 6401 && _Z18TestBitInByteArrayiPhi(struct0205ec34, struct0205ec34 + 0x8c, 0x2a))
        index = 4;
    else
    {
    ...
    _Z14GetVec3ByIndexP8Vec3BA28iPiS1_S1_(lightingInfo, index, &redMix_2c_, &greenMix_30_, &blueMix_34_);
    ...
    _Z34SetupCombatantAllocations_021901acv(func_ov017_0218b5b0());
...
void LightingManager::ProcessZoneChange(Zone3D *newZone)
{
    ...
    else if (info->maybeMode_ == 1)
    {
        ...
        _Z11SetFogStateijjt(true, fog->type, fog->depthShift, fog->offset);
        GX_FOG_COLOR = fog->color | (fog->alpha << 16);
        _Z19CallWithAddr4000360i(fog->densityTable);
        unsigned short edgeColors[8] = {
            edgeColor_, edgeColor_, edgeColor_, edgeColor_,
            edgeColor_, edgeColor_, edgeColor_, edgeColor_
        };
        _Z19CallWithAddr4000330i(edgeColors);
        RenderConfig::SetDiffuseAmbientColors(modelDiffuseColor_, 0, false);
        _Z34SetupCombatantAllocations_021901acv(func_ov017_0218b5b0());
        tint_4a_ = modelDiffuseColor_;
    }
    ...
    {
        _Z11SetFogStateijjt(0, 0, 1, 0x5800);
...
void LightingManager::RecomputeAdvancedLighting()
{
    ...
    {
        int newTimer = lightRGBScaleTransitionTimer_ + _Z18GetField0x3b4ValueP12BattleStruct(battle);
    ...
    if (fogEnabled_)
    {
        _Z11SetFogStateijjt(true, fogInfo_.type, fogInfo_.depthShift, fogInfo_.offset);
        GX_FOG_COLOR = (fogInfo_.color) | (fogInfo_.alpha << 16);
        _Z19CallWithAddr4000360i(fogInfo_.densityTable);
    }
}
...
{
    void* maybeCameraData = _Z18GetField0x3b0ValueP12BattleStruct(GetBattleStruct());
    ...
    _Z27ComputeTwoFromVec3_0202ec84PvP16Vec3copy0202ec84PiS2_(maybeCameraData, &cameraRay, &pixelX, &pixelY);
...
extern "C" LightingManager* _ZN15LightingManagerC1Ev(LightingManager* self)
{
    self->Initialize();
    return self;
}