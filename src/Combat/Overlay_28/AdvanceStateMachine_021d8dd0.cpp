...
extern "C" void _Z14CopyToPaletteBiij(int, int, unsigned int);
...
extern "C" ARM void func_ov028_021d8dd0(unsigned char* self) {
    ...
    if (state == 0) {
        ...
        *reg400100a = (*reg400100a & 0x43) | 0x1000;
        ...
        _Z14CopyToPaletteBiij((int)buf40, 0xa, 2);
        ...
        _Z26TransferToSubBg1ScreenBaseiij((int)buf40, 0, 0x800);
        memset(buf40, 0x11111111, 0x8000);
        CleanInvalidateCacheRange(buf40, 0x8000);
        _Z22TransferSubBg1CharDataiij((int)buf40, 0, 0x8000);
