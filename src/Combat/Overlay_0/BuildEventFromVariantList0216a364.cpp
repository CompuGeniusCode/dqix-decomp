...
ARM int BuildEventFromVariantList0216a364(char* obj, int count) {
    ...
    *(int*)(p + 0x14) = 0;
    ...
    if (count >= 2) {
        int val1 = GetIntFromVariant02030b0c((struct Variant02030b0c*)obj);
        *(unsigned char*)(p + 0xa) = (unsigned char)val1;
        obj += 8;
    }

    if (count >= 3) {
        float f0 = GetTaggedValueAsFloat((struct TaggedValue02030b44*)obj);
        *(float*)(p + 0xc) = f0;
        float f1 = GetTaggedValueAsFloat((struct TaggedValue02030b44*)(obj + 8));
        *(float*)(p + 0x10) = f1;
        float f2 = GetTaggedValueAsFloat((struct TaggedValue02030b44*)(obj + 0x10));
        *(float*)(p + 0x14) = f2;
    }
