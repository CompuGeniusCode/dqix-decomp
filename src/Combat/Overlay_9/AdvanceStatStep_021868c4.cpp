...
// USA: func_ov009_021868c4  (semantic: AdvanceStatStep_021868c4)
extern "C" ARM void func_ov009_021868c4(unsigned char *self) {
	unsigned char step = self[0xc59];
	if (step == 0) {
		func_ov009_02188c2c(self);
		func_ov009_02188d9c(self);
		int idx = self[0xda3];
		unsigned char val = *(self + idx + 0xdac);
	...
	if (result == -3) {
		_Z20SetStatValue021855dcPhi(self, 2);
		return;
	}
