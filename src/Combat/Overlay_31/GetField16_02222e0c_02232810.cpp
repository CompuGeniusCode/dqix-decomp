#include <globaldefs.h>

ARM int GetField16_02222e0c(void);
ARM int CheckField14NonZero_02236f68(void);
ARM void ClearField18_02222e80(void);
ARM void SetField_022274c0_022274c0(int);
ARM int RunThreeSteps_02232844(void);

// USA: func_ov031_02232810  (semantic: CheckFieldsThenRunSteps_02232810)
extern "C" ARM void func_ov031_02232810(void) {
	if (GetField16_02222e0c() == -2) {
		return;
	}
	if (CheckField14NonZero_02236f68() == 1) {
		return;
	}
	ClearField18_02222e80();
	SetField_022274c0_022274c0((int)RunThreeSteps_02232844);
}
