...
void* AllocateAndZero0223cf4c(int size, int align);
extern "C" int func_ov031_0223bfa0(int);
int EndsWithMatch_0223b744(const char* path, void* defaultAllocDesc, int flag);
extern "C" void func_020ca7e8(const void* src, void* dst);
void FreeAndClearPtr_0223cf70(void* p);
...
ARM void* LoadFileWithFallback_0223b61c(const char* path, unsigned int* outSize, int fallback) {
	...
	int allocFlag = (EndsWithMatch_0223b744(path, &data_ov031_0224c56c, 2) != 0) ? -4 : fallback;

	void* buffer = AllocateAndZero0223cf4c(size, allocFlag);
	...
	void* newBuffer = AllocateAndZero0223cf4c(decompSize, fallback);
	func_020ca7e8(buffer, newBuffer);
	FreeAndClearPtr_0223cf70(&buffer);
	return newBuffer;
}
