...
struct ListNode_0215fd4c {
    ...
    char pad30[0x40];
    struct ListNode_0215fd4c* next;
};

// USA: func_ov001_0215fd4c  (semantic: SetNodeFlagAndNotifyVariantD_0215fd4c)
extern "C" ARM int func_ov001_0215fd4c(char* args, int count) {
    ...
    struct ListNode_0215fd4c* node = (struct ListNode_0215fd4c*)GetPointerFromArray0x3c((unsigned char*)base + 0x6c, 2);
    while (node != NULL) {
        if (!(node->flags2e & 1) && node->keyA == key0 && node->keyB == key1) {
            node->flags2e = node->flags2e | 0x380;
