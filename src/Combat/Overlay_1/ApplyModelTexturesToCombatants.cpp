...
// resident. A negative combatant number is folded to 0x9f plus its magnitude, so the two signs
// address two different id ranges; which is which is not established. The 0xac-byte Model3D it
// allocates is never released here, so it seems to sit in an arena somebody else resets.
extern "C" ARM int ApplyModelTexturesToCombatants(char* obj, int count) {
    void* rawArg = obj;
    obj += 0x8;
