...
struct ListNode_0215fa40 {
    ...
    char pad30[0x40];
    struct ListNode_0215fa40* next;
};

// USA: func_ov001_0215fa40  (semantic: SetNodeFlagAndNotifyVariantA_0215fa40)
extern "C" ARM int func_ov001_0215fa40(char* args, int count) {
    ...
    struct ListNode_0215fa40* node = (struct ListNode_0215fa40*)GetPointerFromArray0x3c((unsigned char*)base + 0x6c, 2);
    while (node != NULL) {
        if (!(node->flags2e & 1) && node->keyA == key0 && node->keyB == key1) {
            node->flags2e = node->flags2e | 0x200;
