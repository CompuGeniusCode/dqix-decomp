// USA: func_ov034_022a21a8 = Ov34BackgroundLoader::Process()
// KEEP-NAME: the linked symbol is the mangled C++ member name, not a func_ tag.
...
extern "C"
{
    ...
    void func_020c9b10(void* out);
}

struct AllocatorUnion;
extern "C" void* _Z16AllocateAligned4P14AllocatorUnionj(struct AllocatorUnion* a, unsigned int n);
extern AllocatorUnion data_02114e20;

// Task::operator= is defined in ARM9 main (0x02030310); reference it instead of
// letting the compiler emit a second copy into this overlay.
extern "C" void _ZN16BackgroundLoader4TaskaSERKS0_(BackgroundLoader::Task* dst,
                                                   const BackgroundLoader::Task* src);
#define TASK_ASSIGN(dst, src) _ZN16BackgroundLoader4TaskaSERKS0_(&(dst), &(src))

extern Ov34BackgroundLoader data_ov034_022a2a6c;

// USA: func_ov034_022a21a8
int Ov34BackgroundLoader::Process()
{
    ...
    while (true)
    {
        ...
        {
            Task* pTask = &queuedTasks_[0];
            for (int j = 0; j < numPendingTasks_; j++, pTask++)
            {
                if (pTask->status_ == TaskStatus_Fence)
                {
                    flags_78c_0_ = false;
                    goto end;
                }
                else if (pTask->status_ == TaskStatus_Unallocated &&
                    pTask->type_ == TaskType_LoadFromGP2 &&
                    currentArchiveCRC == func_01ff860c(pTask->outerFilename_))
                {
                    flags_78c_0_ = false;
                    gpcTask = pTask;
                    break;
        ...
        Task inFlightTask;
        TASK_ASSIGN(inFlightTask, *gpcTask); // work with a copy, we'll copy changes back at the end
        flags_78c_3_ = true;
        UnlockResourceMutex();
        char fullName[80];
        ...
        if (inFlightTask.pFileData != NULL)
        {
            ...
            unsigned int y = x ^ ((x >> 24) ^ (x >> 16) ^ (x >> 8));
            int b = (int)(y & 0xff);
            int z = b ^ ((b >> 6) ^ (b >> 4) ^ (b >> 2));
        ...
        {
            TASK_ASSIGN(*pTaskToUpdate, inFlightTask);
        }
        else
        {
            FreeScratchSpace(&inFlightTask.scratchAlloc_);
