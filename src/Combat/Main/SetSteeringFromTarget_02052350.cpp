...
extern "C" ARM void Vector3fix_Normalize(struct Vec3* v, struct Vec3* out);
...
extern "C" ARM void func_02052350(struct Obj02052350* obj) {
    ...
    struct Vec3 diff;
    Vector3fix_Subtract((struct Vec3*)((char*)target + 0x12c), (struct Vec3*)((char*)target + 0x120), &diff);
    int savedY = diff.y;
    diff.y = 0;
    Vector3fix_Normalize(&diff, &diff);
    func_02030e2c((struct Vec3Fixed02030e2c*)&diff, 0x800000, (struct Vec3Fixed02030e2c*)&diff);
    int out1, out2;
    ComputeTwoFromVec3_0202ec84(target, (struct Vec3copy0202ec84*)&diff, &out1, &out2);
    if (out2 < 0xc0 && out2 > 0) {
        int val = fix32_Divide(out2 << 12, 0xbf000);
        obj->f86 = (short)val;
