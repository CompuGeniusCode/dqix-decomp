...
static inline char* GetIndexedEntry020b710c(void* obj, unsigned int index) {
    char* base;
    char* region;
    char* elemPtr;
    struct VarTableHeader020b710c* node;
    ...
    node = (struct VarTableHeader020b710c*)base;
    region = base + 4;
    if (region == NULL) {
        goto noelem;
    }
...
ARM void SetField0xcBits24To29(void* obj, unsigned int index, unsigned int value) {
    unsigned int* entry = (unsigned int*)GetIndexedEntry020b710c(obj, index);
    entry[3] = (entry[3] & ~0x3f000000) | (value << 24);
}
