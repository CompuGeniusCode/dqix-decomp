...
struct NodeF00207a164 { char pad[0xf0]; unsigned short flags; };

// USA: func_0207a164
ARM int OrVariantFlagsIntoNode0207a164(struct Variant02030b0c* arg0, int count) {
    char* base = (char*)&data_02108e40;
    SearchNode0207afbc** head = *(SearchNode0207afbc***)(base + 8);
    if (head == NULL) return 0;
    const char* name = *(const char**)(base + 4);
    if (name == NULL) return 0;
    struct NodeF00207a164* node = (struct NodeF00207a164*)FindNodeByName0207afbc(head, name);
    ...
    if (count >= 2) {
        v = _ZNK6Script9Parameter5ToIntEv(arg0 + 1);
        if (v <= 8) node->flags |= v;
        return 1;
    } else {
        return 1;
