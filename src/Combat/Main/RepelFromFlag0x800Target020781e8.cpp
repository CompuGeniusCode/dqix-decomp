...
extern "C" int func_020c338c(int x);
...
ARM int RepelFromFlag0x800Target020781e8(struct Entity020781e8* e) {
    ...
    if (!CheckField0NonZero((int*)g) || GetSearchStructCurrentArrEntry((struct SearchStruct0202c1a4*)g) == 0) {
        ...
        struct Vec3 delta;
        SubtractVec3(&posCopy, &e->f44, &delta);
        func_020c2f18(&delta, &delta);
        delta.x = -delta.x;
        delta.y = -delta.y;
        delta.z = -delta.z;
        scale = func_020c338c(delta.x);
        SetVecYByMode02033834((struct Obj02033834*)e, scale);
        delta.y = 0;
        ScaleVec3Fixed02030e2c((struct Vec3Fixed02030e2c*)&delta, 0x5000, (struct Vec3Fixed02030e2c*)&delta);
        AddVec3(&e->f44, &delta, &e->f158);
    }

    e->fb4 = 0x1c2;
    e->fb2 = 0x1c2;
    SetByteIfChanged02033b68((struct Obj02033b68*)e, 1);
    ClearBitsInField0x6c((unsigned char*)e, 0x80);
    e->f17a = 0;

    if (CheckField0NonZero((int*)g) && GetSearchStructCurrentArrEntry((struct SearchStruct0202c1a4*)g) == 0) {
        int c1 = GetU16At0x6((struct U16Field0x6_020375f8*)e);
        int f2val = e->f2;
