...
extern "C" int func_0200cd1c(unsigned int lo, unsigned int hi, unsigned int divLo, unsigned int divHi);
extern "C" int func_0200cd2c(unsigned int lo, unsigned int hi, unsigned int divLo, unsigned int divHi);
...
ARM void SplitTimestampIntoDateAndTime(void* datePtr, struct TimeOfDay020cfaf4* timePtr, long long timestamp) {
    ...
    int secOfDay = func_0200cd1c(lo, hi, 0x15180, 0);
    SplitSecondsIntoTimeOfDay(timePtr, secOfDay);
    int days = func_0200cd2c(lo, hi, 0x15180, 0);
