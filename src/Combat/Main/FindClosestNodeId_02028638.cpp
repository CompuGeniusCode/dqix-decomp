...
struct SearchCtx_02028638 {
    unsigned char pad0[2];
    unsigned char count;
    unsigned char pad1[1];
    struct Node0x10_02028638* nodes;
};

// USA: func_02028638  (semantic: FindClosestNodeId_02028638)
extern "C" ARM int func_02028638(struct SearchCtx_02028638* ctx, struct Vec3s32_020c3030* pos) {
    if (ctx->count == 0) return 0;

    struct Node0x10_02028638* nodes = ctx->nodes;
    struct Vec3s32_020c3030 v0;
    v0.x = (int)nodes[0].x << 12;
    v0.y = (int)nodes[0].y << 12;
    v0.z = (int)nodes[0].z << 12;
    int bestId = 0;
    int bestDist = Vector3fix_Distance(pos, &v0);
    ...
    for (int i = 1; i < ctx->count; i++) {
        ...
        int d = Vector3fix_Distance(pos, &v);
        if (d < bestDist) {
            bestId = nodes[i].id;
            bestDist = d;
        }
