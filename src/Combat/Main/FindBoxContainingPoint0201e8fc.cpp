...
ARM void MulVec3MtxTranslate020c2034(struct FixedVec3_2034* v, struct FixedMtx3T_2034* m, struct FixedVec3_2034* out);
...
ARM void* FindBoxContainingPoint0201e8fc(struct Container0201e8fc* obj, struct Vec3* pt) {
    ...
    for (i = 0; i < obj->count; i++) {
        ...
        SubtractVec3(pt, (struct Vec3*)e, &diff);
        angle = func_02030f30(-(short)e->angle);
        BuildRotationMatrixY(&mtxA, angle);
        mtxB = mtxA;
        MulVec3MtxTranslate020c2034((struct FixedVec3_2034*)&diff, (struct FixedMtx3T_2034*)&mtxB, (struct FixedVec3_2034*)&diff);
        AddVec3(&diff, (struct Vec3*)e, &diff);
        ...
        if (CheckPointOutsideBounds02031118((struct Point02031118*)&diff, &bounds) != 0)
            return e;
    }
    return 0;
}
