...
struct RngHolder_021da248 { struct Random* rng; };

// USA: func_ov024_021da248
ARM int RandomScaleField6cMaxOfTwo6x160_021da248(struct RngHolder_021da248* holder, unsigned short category, int unused1, int unused2, int unused3, int fallback) {
    int inRange = 0;
    if (category <= 3) {
        inRange = 1;
    }
    ...
    float randomFactor1 = NextRandomFloatBetween(rng, 0.9f, 1.0f);
    float a = 6.0f * value * randomFactor1;
    float randomFactor2 = NextRandomFloatBetween(rng, 0.9f, 1.1f);
    float b = 160.0f * randomFactor2;
    if (a < b) {
        a = b;
    }
    return (int)a;
