...
extern "C" ARM int func_ov024_021ef6c4(struct Obj_021ef6c4* obj, int unused1, int unused2, int* outCount, short* outArray) {
    ...
    for (int i = 0; i < count; i++) {
        ...
        if (!c) continue;
        if (*(unsigned short*)((char*)c->currentStats + 0xa) == 0) continue;
        struct Bits58_021ef6c4* bits = (struct Bits58_021ef6c4*)((char*)c->currentStats + 0x58);
        if (bits->fieldZ <= -2) continue;
        if (IsCombatantFlagMask512_021eda60(c)) continue;
        int byteVal = *(unsigned char*)((char*)c->currentStats + 0x50);
