...
extern "C" ARM int func_ov024_021f5344(struct Obj_021f5344* obj, short id, int unused2, int* outFlag, short* outId) {
    struct CombatantStruct* c = GetCombatantWithFlag0x400ByID(obj->field0, id);
    if (!c) return 0;
    ...
    for (int i = 0; i < count; i++) {
        struct CombatantStruct* c2 = GetCombatantByID(obj->field0, buf.v[i]);
        if (!c2) continue;
        if (!CheckFlag0x14Bit0x10Set((unsigned char*)c2->currentStats)) matched++;
    }
