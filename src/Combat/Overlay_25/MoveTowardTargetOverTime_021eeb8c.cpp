...
extern "C" ARM int func_ov025_021eeb8c(struct MoveRequest_021eeb8c* req) {
    ...
    int dist = Distance3D020c3030((struct Vec3s32_020c3030*)&curPos, (struct Vec3s32_020c3030*)&req->target);
    float ratio = (float)(unsigned int)elapsed / (float)req->remaining;
    int step = (int)(((long long)dist * (int)(ratio * 4096.0f) + 0x800) >> 12);
    struct Vec3 diff;
    SubtractVec3(&req->target, &curPos, &diff);
    int scale2 = HwDivideRounded020c2bf4(step, dist);
    ScaleVec3Fixed02030e2c((struct Vec3Fixed02030e2c*)&diff, scale2, (struct Vec3Fixed02030e2c*)&diff);
    AddVec3(&curPos, &diff, &curPos);
