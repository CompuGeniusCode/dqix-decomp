...
ARM void* GetFileDataMaybeGp020e0574(const char* path, unsigned int* outSize, void* unused, const char* gp2Path) {
    ...
    *outSize = 0;
    ...
    unsigned int* sizeArg = outSize;
    if (gp2Path == 0) {
        LoadFileIntoMemory(buf, data_0211e33c, sizeArg);
        if (*sizeArg != 0) return data_0211e33c;
    } else {
        result = ExtractFileFromGP2(gp2Path, buf, sizeArg);
